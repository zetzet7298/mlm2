...
class CheckoutController extends Controller
{
    ...
    public function index()
    {
        if (Cart::instance('default')->count() > 0) {
            $subtotal = floatval(Cart::instance('default')->subtotal()) ?? 0;
    ...
    public function store(CheckoutRequest $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect('index')->with('success', 'Your order is completed successfully!');
        } catch (Exception $e) {
    ...
    private function getNumbers()
    {
        ...
        if(gettype($newSubtotal) == 'string'){
            $newSubtotal = floatval(str_replace(',', '', $newSubtotal));
            }
        // $newTax = $newSubtotal * $tax;
        $newTotal = $newSubtotal + 0;
        return collect([
            // 'tax' => $tax,
            'discount' => $discount,
            'code' => $code,
